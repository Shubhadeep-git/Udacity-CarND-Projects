...
UKF::UKF() {
  ...
  */
	//boolean variable for initialization
	is_initialized_ = false;
	
	//variable to store dimension of state vector
	n_x_ = 5;
	
	//variable to store dimension of augmented state vector
	n_aug_ = 7;
	
	//Sigma point spreading parameter
	lambda_ = 3 - n_aug_;
	
	//predicted Augmented sigma point matrix
  ...
  // the initial value of NIS for radar
  NIS_radar_ = 0;

  // the initial value of NIS for lidar
  NIS_lidar_ = 0;
}
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
	...
	//time difference between consecutive measurements
...
void UKF::Prediction(double delta_t) {
  ...
  */
	...
	//sqrt matrix of augmented state covariance matrix
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
	...
	MatrixXd Zsig = MatrixXd(n_z_,Xsig_pred_.cols());
  for(int i=0; i<Xsig_pred_.cols(); i++)
	{
		Zsig.col(i) = Xsig_pred_.col(i).head(2);
	}

  //predicted mean of sigma points
  ...
  //Kalman Gain
	...
	NIS_lidar_ = (z - z_pred).transpose() * S_pred.inverse() * (z - z_pred);
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  //predicted mean of sigma points
	...
	VectorXd del_z = (z - z_pred);
	//angle normalization
	del_z(1) = atan2(sin(del_z(1)),cos(del_z(1)));
	...
	//Calculating NIS for lidar
	NIS_radar_ = del_z.transpose() * S_pred.inverse() * del_z;
}
